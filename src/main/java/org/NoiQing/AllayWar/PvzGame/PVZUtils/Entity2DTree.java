...
public class Entity2DTree {
    ...
    private Node findNearest(Node node, Entity entity, Node nearest, boolean xAxis) {
        ...
        if (nearest == null || distance(entity, node.entity) < distance(entity, nearest.entity)) {
            nearest = node;
        }
        ...
        if (Math.abs(xAxis ? entity.getLocation().getX() - node.entity.getLocation().getX() : entity.getLocation().getZ() - node.entity.getLocation().getZ()) < distance(entity, nearest.entity)) {
