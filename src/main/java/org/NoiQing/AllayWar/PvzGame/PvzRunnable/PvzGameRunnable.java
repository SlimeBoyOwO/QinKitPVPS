...
public class PvzGameRunnable extends BukkitRunnable {
        ...
        for(World w : Bukkit.getWorlds()) {
            for(Entity e: w.getEntitiesByClass(LivingEntity.class)) {
                ...
                if(PVZFunction.isBullet(e)) {
                    int bulletCD = PvzEntity.getPlantAttackCD(e);
                    if(bulletCD >= 10) {
                        if(e.hasGravity()) {
                    ...
                    if(e.getScoreboardTags().contains("pea_bullet")) {
                        ...
                        e.getWorld().spawnParticle(Particle.ITEM_SLIME,e.getLocation(),10,0,0,0,0,null,false);
                    } else if(e.getScoreboardTags().contains("ice_pea_bullet")) {
                        hitEntity.damage(4, shooter);
                        e.getWorld().spawnParticle(Particle.ITEM_SNOWBALL,e.getLocation(),10,0,0,0,0,null,false);
                        ...
                        e.getWorld().spawnParticle(Particle.ITEM_SLIME,e.getLocation(),10,0,0,0,0,null,false);
                    } else if(e.getScoreboardTags().contains("corn_bullet")) {
                        hitEntity.damage(4, shooter);
                        e.getWorld().spawnParticle(Particle.ITEM,e.getLocation(),10,0.5,0,0.5,0,new ItemStack(Material.GOLD_INGOT),false);
                    } else if(e.getScoreboardTags().contains("melon_bullet")) {
                        hitEntity.damage(16, shooter);
                        for(Entity nearby : hitEntity.getNearbyEntities(1,1,1))
                            if(isEnemy(e,nearby)) hitEntity.damage(8, shooter);
                        e.getWorld().spawnParticle(Particle.ITEM,e.getLocation(),20,0.5,0.25,0.5,0,new ItemStack(Material.MELON_SLICE),false);
                    }
    ...
    private LivingEntity setPlantTarget(Mob mob,LivingEntity target) {
        ...
        if (target == null)
            newTarget = findNearestEnemy(mob,30,false);
        else newTarget = findNearestEnemy(mob,30,true);
    ...
    private Entity generateBullet(Mob mob, LivingEntity target, Boolean isPea) {
        ...
        EnderPearl pea = mob.getWorld().spawn(fireLocation, EnderPearl.class);
    ...
    private LivingEntity findNearestEnemy(Entity mob, double findDistance, boolean once) {
