...
public class AWFunction {
    ...
    public static Location getTeamRespawnPoint(String team) {
        ...
        QinMap m = QinKitPVPS.getPlugin().getKitGame().getMaps().getAllayQinMapByMapID(allayGame.getChooseMapID());
        int x = switch (team) {
            case "Yellow" -> 1;
            case "Blue" -> 2;
            case "Green" -> 3;
    ...
    public static void givePlayerRespawnItem(Player p) {
        ...
        AllayGame allayGame = QinKitPVPS.getPlugin().getGames().getAllayGame();
        if(t != null) {
            sharpnessLevel = allayGame.getTeamLevels(t.getTeamName()).getOrDefault("Sharpness",0);
            protectionLevel = allayGame.getTeamLevels(t.getTeamName()).getOrDefault("Protection",0);
            haveRaider = allayGame.getTeamLevels(t.getTeamName()).getOrDefault("HaveRaider",0);
        }
