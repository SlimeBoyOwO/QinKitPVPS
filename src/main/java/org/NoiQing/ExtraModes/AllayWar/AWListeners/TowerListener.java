...
import org.NoiQing.ExtraModes.AllayWar.AWUtils.AWBossBars;
...
public class TowerListener implements Listener {
    ...
    public void onProjectileHit(ProjectileHitEvent event) {
        if(!QinKitPVPS.getPlugin().getGames().getAllayGame().isRunning()) return;
        QinTeam attackTeam = QinTeams.getEntityTeam(event.getEntity());
        QinTeam victimTeam = QinTeams.getEntityTeam(event.getHitEntity());
    ...
    //防止游戏对局中队友伤害
    @EventHandler
    public void onTeamMateHurt(EntityDamageByEntityEvent e) {
        if(!QinKitPVPS.getPlugin().getGames().getAllayGame().isRunning()) return;
        Entity victim = e.getEntity();
        Entity damager = e.getDamager();
        QinTeam attackTeam = QinTeams.getEntityTeam(e.getDamager());
        QinTeam victimTeam = QinTeams.getEntityTeam(e.getEntity());
        ...
        if(AWFunction.isAllayTower(victim)) {
            ...
            if(e.getDamager() instanceof LivingEntity lv) {
                ...
                } else if(attackTeam != null && attackTeam.equals(victimTeam)) {
                    e.setDamage(0);
                }
            }
            return;
        }
        if(victimTeam instanceof Player && attackTeam != null && attackTeam.equals(victimTeam)) {
            e.setCancelled(true);
        }
    }
    ...
    public void onAllayDamaged(EntityDamageByEntityEvent e) {
        ...
        if(entity.getScoreboardTags().contains("allay_army")) {
            ...
            if(murder.getScoreboardTags().contains("allay_sniper_arrow")) {
    ...
    public void onTankDamaged(EntityDamageByEntityEvent e) {
        if(!AWFunction.isTank(e.getEntity())) return;
    ...
    public void onTowerDead(EntityDeathEvent e) {
        if(e.getEntity() instanceof Allay a && isAllayTower(a)) {
            ...
            if(Function.getNameWithoutColor(a.getCustomName()).equals("۞ 生命核心 ۞")) {
                if(aTeam != null && allayGame.getTeamLevels(aTeam.getTeamName()) != null) {
                    allayGame.getTeamLevels(aTeam.getTeamName()).put("HaveBase",0);
                ...
                if(aTeam != null && allayGame.getTeamLevels(aTeam.getTeamName()) != null) {
                    allayGame.getTeamLevels(aTeam.getTeamName()).put("HaveRaider",0);
    ...
    public void onPlayerEnterRadar(PlayerInteractEvent e) {
        ...
        if(t == null) return;
        AllayGame allayGame = QinKitPVPS.getPlugin().getGames().getAllayGame();
        if(allayGame.getTeamLevels(t.getTeamName()).get("HaveRaider") == 0) {
            Function.sendPlayerSystemMessage(p,"队伍雷达信号已经丢失！");
            e.setCancelled(true);
            return;
        }
        Skeleton s = p.getWorld().spawn(p.getLocation(),Skeleton.class);
        Function.setMobEquipment(s,new ItemStack(Material.AIR));
        t.addTeamEntities(s);
    ...
    private void summonArmy(Player p, String armyName, Location loc) {
        switch (armyName) {
            ...
            case "丧尸巨人" -> {
                ...
                z.setAdult();
                Function.setMobEquipment(z,new ItemStack(Material.AIR), new ItemStack(Material.DIAMOND_HELMET));
            }
            ...
            case "步枪士兵" -> {
                for(int i = 0; i < 3; i++) {
                    ...
                    z.setAdult();
                    z.setHealth(20);
                    Function.setMobEquipment(z,new ItemStack(Material.IRON_HOE),new ItemStack(Material.IRON_HELMET));
                }
            }
            ...
            case "工程师" -> {
                ...
                z.setAdult();
                z.setHealth(20);
                Function.setMobEquipment(z,new ItemStack(Material.ORANGE_CONCRETE),new ItemStack(Material.IRON_HELMET));
            }
            ...
            case "自爆步兵" -> {
                for(int i = 0; i < 2; i++) {
                    ...
                    Function.setMobEquipment(z,new ItemStack(Material.TNT),helmet);
                }
            }
            ...
            case "地狱蛮兵" -> {
                for(int i = 0; i < 2; i++) {
                    ...
                    Function.setMobEquipment(z,item,helmet,chestplate,leggings,boots);
                }
            }
        }
    }
    ...
    private void summonTankCore(Ravager z) {
        ...
        Objects.requireNonNull(z.getAttribute(Attribute.GENERIC_KNOCKBACK_RESISTANCE)).setBaseValue(0.8);
    ...
    private boolean summonAllayAndTower(Player p, String towerType, Location loc) {
        ...
        switch (towerType) {
            ...
            case "雷达" -> {
                ...
                AllayGame allayGame = QinKitPVPS.getPlugin().getGames().getAllayGame();
                if (team != null) {
                    allayGame.getTeamLevels(team.getTeamName()).put("HaveRaider",1);
